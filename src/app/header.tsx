      ...
      <div className="container flex h-14 items-center">
            ...
            <NavigationMenuList>
              {navItems.map((item) => (
                <NavigationMenuItem key={item.title}>
                  ...
                  rounded-md bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground 
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <SheetTrigger asChild>
            <Button
              variant="ghost"
              className="mr-2 px-0 text-base hover:bg-transparent focus-visible:bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 md:hidden flex items-center" // Added flex items-center
            >
...
function MobileNav() {
  return (
    <div className="flex flex-col space-y-3">
      ...
      {navItems.map((item) => (
        <Link key={item.title} href={item.href} className="text-foreground/70 transition-colors hover:text-foreground">
          {item.title}
        </Link>
      ))}
